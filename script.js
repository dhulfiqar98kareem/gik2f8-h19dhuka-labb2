...
function renderTask({id, title, description, dueDate, completed}) {
  ...
  */
  ...
  let checked = "";
  let taskCompletedStyle = "";

  if (completed) {
    checked = "checked";
    taskCompletedStyle = `bg-red-300`
  }
  ...
  let html = `
    <li class="bg-white bg-opacity-75 select-none mt-2 p-3 rounded border-neutral-400 border border-solid shadow ${taskCompletedStyle}">
  ...
  html += `
      <div class="flex justify-end">
        <label for="completed__${id}">utförd:</label>
...
/* Function to update a task. Takes in the task's id and the new data. */
function updateTask(id, data) {
  // Call the update function from the API and pass in the id and new data
  api.update(id, data).then((result) => {
    // After the task is updated, re-render the task list
    renderList();
  });
}
