...
app.patch('/tasks/:id', async (req, res) => {
  try {
    // Read the existing JSON file containing all tasks 
    const tasksJson = await fs.readFile('./tasks.json');
    // Parse the JSON data and store it in a variable
    const taskCollection = JSON.parse(tasksJson);
    //Find the task that needs to be updated by comparing the passed id in the url with the id of each task
    const targetTask = taskCollection.find(task => task.id == req.params.id);
    // Store the new data that came with the request in a variable
    const updatedData = req.body;

    if (targetTask) {
      // Overwrite the existing task data with the new data
      Object.assign(targetTask, updatedData);
      // Write the updated collection of tasks to the json file
      await fs.writeFile('./tasks.json', JSON.stringify(taskCollection));
      
      res.send({message: `Task with id ${targetTask.id} updated`});
    } else {
      // If no task was found to update, return an error
      res.status(404).send({error: 'No task to update'});
    }
  } catch (error) {
